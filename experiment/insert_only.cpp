...
std::string InsertOnly::get_file_name(std::string filePath)
{
    ...
    if (lastSeparatorPos != std::string::npos) {
        ...
        if (lastPeriodPos != std::string::npos) {
            ...
            LOG("File has no extension."<<fileName);
           // std::cout << "File Name: " << fileName << std::endl;
        } else {
           // std::cout << "File has no extension." << std::endl;
            LOG("File has no extension.");
...
InsertOnly::InsertOnly(std::shared_ptr<gfe::library::UpdateInterface> interface, std::shared_ptr<gfe::graph::WeightedEdgeStream> graph, int64_t num_threads) :m_interface(interface), m_stream(graph), m_num_threads(num_threads) {
...
static void run_sequential(library::UpdateInterface* interface, graph::WeightedEdgeStream* graph, uint64_t start, uint64_t end, int thread_id){
    for(uint64_t pos = start; pos < end; pos++){
        auto edge = graph->get(pos);
        [[maybe_unused]] bool result = interface->add_edge_v2(edge,thread_id);
    ...
    for(int64_t i = 0; i < m_num_threads; i++){
        threads.emplace_back([this, &start_chunk_next](int thread_id)
        {
            ...
            while( (start = start_chunk_next.fetch_add(m_scheduler_granularity)) < size ){
                ...
                run_sequential(interface, graph, start, end, thread_id);
            }
...
chrono::microseconds InsertOnly::execute()
{
    ...
    build_service.stop();
    timer.stop();
    //LOG("Insertions performed with " << m_num_threads << " threads in sec " << timer.seconds());
...
void InsertOnly::save(bool is_file, bool is_timestamp)
{
    ...
    else
    {
        ...
        {
           // LOG("Only insertion scalability throughput");
            std::ofstream outfile;
            if(configuration().s)
            {
                std::cout<<"scalability analysis"<<std::endl;

                outfile.open(commonDir + "insertion-only-scalability.txt",std::ios_base::app); // append instead of overwrite
                int system_id = 0;
                ...
                outfile << system_id << " " << configuration().m_num_threads_write<<" "<< m_time_insert<< "\n";

            }
            else
            {
                outfile.open(commonDir + "insertion-only.txt",
                ...
                if (is_timestamp) {
                    std::string file_path_ord = get_file_name(configuration().m_path_graph_to_load) + "_ordered";
                    outfile << system_id << " " << m_time_insert << " " << m_stream->num_edges() << " " << file_path_ord
                            << "\n";

                }
                else {
                    outfile << system_id << " " << m_time_insert << " " << m_stream->num_edges() << " "
                            << get_file_name(configuration().m_path_graph_to_load) << "\n";
                }
            }
            outfile.close();
        }
        ...
        {
            LOG("Only analytics part, no insertion to save in file");
        }
