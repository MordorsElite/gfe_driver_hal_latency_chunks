...
void Aging2Result::save(common::Database* handle, bool is_database) {
    ...
    else
    {
        ...
        std::cout<<"Number of updates operation: "<<m_num_operations_total<<std::endl;
        std::cout<<"completion time: "<<m_completion_time<<std::endl;
        double sum = 0;
        //double total_num = m_progress.size();

        double num_op   = m_num_operations_total;
        double com_time = m_completion_time;
       //  for (uint64_t i = 0, sz = m_progress.size(); i < sz; i++)
        //{
          //  sum += m_progress[i];
        //}

        //std::cout<<"Number of updates operation per second: "<<(num_op/com_time)<<std::endl;
       // std::cout<<"Number of total operation: "<<(m_num_operations_total/total_num)<<std::endl;

    }
}

void Aging2Result::save(std::shared_ptr<common::Database> db, bool is_database){
    save(db.get(), is_database);
}
