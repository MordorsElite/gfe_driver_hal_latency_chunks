...
std::ostream& operator<<(std::ostream& out, const ExecStatistics& stats){
    ...
    outfile1.open("/local/ghufran/graph_result_halb_mixed-workload_results2.txt", std::ios_base::app); // append instead of overwrite
    outfile1<<"N:"<<stats.m_num_trials<<" median"<< stats.m_median<<"\n";
    return out;
}
