 ...
 ****************************************************************************/
    ...
    std::chrono::microseconds GraphalyticsSequential::execute(){
        ...
        for(uint64_t i = 0; i < m_num_repetitions; i++)
        {
            ...
            if(m_properties.cdlp.m_enabled){
                // #### bfs 0, pagerank 1, sssp 2, wcc 3, cdlp 4
    ...
    void GraphalyticsSequential::report_result_in_file()
    {
        std::ofstream outfile1;
        outfile1.open("/local/user/analytics_only_result.txt", std::ios_base::app); // append instead of overwrite
        int system_id = 0;
        ...
        else if (configuration().get_library_name() == "teseo.13" || configuration().get_library_name() == "teseo-lcc.13")
        {
            // teseo
            system_id = 2;
        }
        else if(configuration().get_library_name() == "teseo-lcc-dv.13" || configuration().get_library_name() == "teseo-dv.13" )
        {
            // 4 id for dense teseo results
            system_id = 4;
        }
        else
        {
        ...
        int algo_id = 0;
        // bfs 0, pagerank 1, sssp 2, wcc 3, cdlp 4, lcc 5
        ...
        if(!m_exec_bfs.empty())
        {
            ...
            ExecStatistics stats { m_exec_bfs };
            cout << ">> BFS " << stats << "\n";
            outfile1<<system_id<<" "<<algo_id<<" "<<stats.m_median<<" "<<get_file_name(configuration().m_path_graph_to_load)<<"\n";
        }
        if(!m_exec_cdlp.empty())
        {
            algo_id = 4;
