...
void Aging2Master::load_edges(){
    ...
    src_timestamp = 1267530904;
    reader::graphlog::EdgeLoader loader(handle);
    uint64_t num_edges = loader.load(array1, array_sz / 3);

    // for timestamp generation
    uint64_t* timestamp_arr = static_cast<uint64_t *>(malloc(num_edges*sizeof(uint64_t)));
    for(int64_t l=0;l<num_edges;l++)
    {
        timestamp_arr[l] = src_timestamp + l;
    }
        src_timestamp = num_edges + 1;
    while( num_edges > 0 )
    {
        // partition the batch among the workers
        for(auto w: m_workers) w->load_edges(array1,timestamp_arr, num_edges);
...
void Aging2Master::do_run_experiment(){
    ...
    cooloff(start_time);//THREADS_READ
    ...
    do
    {
        ...
        if(!done){
            ...
            if(measure_memfp && (/* first tick */ (m_results.m_progress.size() == 1) || tp - last_memory_footprint_recording >= 10s))
            {
                ...
                if(m_parameters.library_name == "our")
                {
                    memfp_process = (memfp_process - m_parameters.m_library->on_thread_destroy(-1));
                }
