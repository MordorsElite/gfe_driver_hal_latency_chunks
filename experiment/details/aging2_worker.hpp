...
class Aging2Worker {
    ...
    struct Task { TaskOp m_type;uint64_t* Srctimestamps; uint64_t* m_payload; uint64_t m_payload_sz; };
    ...
    void main_load_edges(uint64_t* src_timestmaps, uint64_t* edges, uint64_t num_edges);
    ...
    void set_task_async(TaskOp task_type,uint64_t* timestamps = nullptr, uint64_t* payload = nullptr, uint64_t payload_sz = 0);
...
public:
    ...
    void load_edges(uint64_t* edges,uint64_t* timestamps, uint64_t num_edges);
