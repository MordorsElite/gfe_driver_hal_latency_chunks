...
void Aging2Worker::load_edges(uint64_t* edges,uint64_t* timestamps, uint64_t num_edges){
    set_task_async(TaskOp::LOAD_EDGES,timestamps, edges, num_edges);
}
...
void Aging2Worker::remove_vertices(uint64_t* vertices, uint64_t num_vertices){
    set_task_async(TaskOp::REMOVE_VERTICES,nullptr, vertices, num_vertices);
}

void Aging2Worker::set_latencies(uint64_t* array_insertions, uint64_t* array_deletions){
    set_task_async(TaskOp::SET_ARRAY_LATENCIES, nullptr,array_insertions, /* hack */ reinterpret_cast<uint64_t>(array_deletions));
}

void Aging2Worker::set_task_async(TaskOp type, uint64_t* timestamps, uint64_t* payload, uint64_t payload_sz){
    { // restrict the scope
        ...
        m_task = Task { type,timestamps, payload, payload_sz };
    }
    m_condvar.notify_all();
}
...
void Aging2Worker::main_thread(){
    ...
    do {
        ...
        case TaskOp::LOAD_EDGES:
            main_load_edges(task.Srctimestamps,task.m_payload, task.m_payload_sz);
            break;
...
void Aging2Worker::main_load_edges(uint64_t* src_timestamps, uint64_t* edges, uint64_t num_edges){
   ...
   // int64_t * __restrict timestampsynthetic = malloc(num_edges*sizeof());
    ...
    for(uint64_t i = 0; i < num_edges; i++){
        if(static_cast<int>((sources[i] + destinations[i]) % modulo) == m_worker_id)
        {
            ...
            last->emplace_back(sources[i], destinations[i], weight,src_timestamps[i]);
...
void Aging2Worker::graph_execute_batch_updates0(graph::WeightedEdge* __restrict updates, uint64_t num_updates){
    for(uint64_t i = 0; i < num_updates; i++){
        ...
        { // deletion
            graph_remove_edge<with_latency>(updates[i]);
        }
...
void Aging2Worker::graph_insert_edge(graph::WeightedEdge edge){
    ...
    if(with_latency == false){
        ...
        while ( ! m_library->add_edge_v2(edge,this->m_worker_id-1) ) { /* nop */ };

    } else { // measure the latency of the insertion
        chrono::steady_clock::time_point t0, t1;
        do {
            t0 = chrono::steady_clock::now();
        } while ( ! m_library->add_edge_v2(edge,this->m_worker_id-1) );
...
void Aging2Worker::graph_remove_edge(graph::Edge edge, bool force){
    ...
    if(with_latency == false){
        ...
        if(!force){
            m_library->remove_edge(edge,(this->m_worker_id-1));
        } else { // force = true
            while( ! m_library->remove_edge(edge,(this->m_worker_id-1)) ) /* nop */ ;
        }
