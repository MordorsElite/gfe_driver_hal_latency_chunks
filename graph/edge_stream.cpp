...
namespace gfe::graph {
  void  calculate_Degreebyvector(WeightedEdge edge)
  {
...
WeightedEdgeStream::WeightedEdgeStream(const std::string& path)
{
    ...
    while(reader->read(edge))
    {
     ...
     //   srcdegree[edge.m_source] = srcdegree[edge.m_source] + 1;
    ...
    //    continue;
        ...
        srcTimestamp.emplace_back(edge.srcTimestamp);
...
bool WeightedEdgeStream::generate_synthetic_timestamp()
{
    // 01-01-2016 00-00-00 (h-m-s)
    time_t now = 1451602800; //time(0);
    for(int i=0;i<m_num_edges;i++)
    {
        srcTimestamp.emplace_back(now);
        now++;
    }
return true;
}
...
WeightedEdge WeightedEdgeStream::get(uint64_t index) const {
    ...
    return WeightedEdge { m_sources->get_value_at(index), m_destinations->get_value_at(index), m_weights[index],srcTimestamp[index]};
}
