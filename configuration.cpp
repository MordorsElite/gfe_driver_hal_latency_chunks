...
void Configuration::initialise(int argc, char* argv[]){
    ...
    options.add_options("Generic")
        ...
        ("ooo", "1-2-3, if 1 its simple out-of-order updates else 2 for duplicate edges and 3 for edge count between different systems", value<int>()->default_value(to_string(0)))
        ("s", "1, if 1 means cal scalability of dataset otherwise simple insertion", value<bool>()->default_value("false"))
        ("ids", "Enable duplicate support in our system", value<bool>()->default_value("false"));
    ...
    try {
        ...
        if(result["ooo"].count() > 0)
        {
            out_of_order_workload = result["ooo"].as<int>();
        }
        if(result["s"].count() > 0)
        {
            s = result["s"].as<bool>();
        ...
        {
            m_is_duplicate_support = result["ids"].as<bool>();
        }
...
void Configuration::save_parameters() {
    ...
    params.push_back(P{"s", to_string(s)});
