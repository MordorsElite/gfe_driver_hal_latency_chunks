...
bool LLAMAClass::add_edge_v2(graph::WeightedEdge e,, int thread_id){
    shared_lock<shared_mutex_t> cplock(m_lock_checkpoint); // forbid any checkpoint now
    COUT_DEBUG("edge: " << e);
...
bool LLAMAClass::remove_edge(graph::Edge e, int thread_id){
    shared_lock<shared_mutex_t> cplock(m_lock_checkpoint); // forbid any checkpoint now
    COUT_DEBUG("edge: " << e);
...
bool LLAMAClass::remove_edge_v2(gfe::graph::WeightedEdge e, int thread_id)
{
