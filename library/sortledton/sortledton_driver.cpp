...
namespace gfe::library {
    ...
    SortledtonDriver::SortledtonDriver(bool is_graph_directed, size_t properties_size, int block_size) : tm(1),
      ...
      ds = new VersioningBlockedSkipListAdjacencyList(512, properties_size, tm);
    }
    ...
    uint64_t SortledtonDriver::on_thread_destroy(int thread_id) {
 ...
 */
    ...
    bool SortledtonDriver::add_edge_v2(gfe::graph::WeightedEdge e,int thread_id) {
      assert(!m_is_directed);
...
//      tm.transactionCompleted(tx);
    ...
    bool SortledtonDriver::remove_edge(gfe::graph::Edge e, int thread_id) {
      assert(!m_is_directed);
    ...
    bool SortledtonDriver::remove_edge_v2(gfe::graph::WeightedEdge e, int thread_id)
    {
        assert(!m_is_directed);
 ...
 *****************************************************************************/
    ...
    void SortledtonDriver::lcc(const char *dump2file)
    {
      ...
      auto lcc_values = GFELCC::execute(tx);
        //double sum = 0;
       // for(int i=0;i<lcc_values.size();i++)
        //{
          //  sum += lcc_values[i];
        //}
        //std::cout<<"sum :"<<sum<<std::endl;

        //auto lcc_values_1 = LCC::lcc_merge_sort(tx);
        //double sum1 = 0;
        //for(int i=0;i<lcc_values_1.size();i++)
        //{
          //  sum1+=lcc_values_1[i];
        //}
