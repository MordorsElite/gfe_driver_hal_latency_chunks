...
#define COUT_DEBUG(msg)
    ...
    class SortledtonDriverV2 : public virtual UpdateInterface, public virtual GraphalyticsInterface {
        SortledtonDriverV2(const SortledtonDriverV2 &) = delete;
        SortledtonDriverV2 &operator=(const SortledtonDriverV2 &) = delete;

     protected:
      const bool m_is_directed;
      sortledton::storage::VersionedGraphStore ds;
      sortledton::storage::GraphStorageForwarder tx;
        ...
        template <typename T>
        vector<pair<uint64_t, T>> translate(sortledton::storage::GraphStorageForwarder& tx, vector<T>& values) {
          int N = values.size();

          vector<pair<sortledton::vertex_id_t , T>> logical_result(N);
...
#pragma omp parallel for
    ...
    public:
        ...
        SortledtonDriverV2(bool is_graph_directed, int block_size);
      ...
      */
      virtual bool has_edge(uint64_t source, uint64_t destination) const;
        ...
        */
        virtual bool add_edge_v2(gfe::graph::WeightedEdge e, int thread_id);
        ...
        virtual bool remove_edge_v2(graph::WeightedEdge e, int thread_id);
