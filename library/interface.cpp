...
#endif
#if defined(HAVE_HALB)
#include "HALB/halb_driver.hpp"
#endif
...
std::unique_ptr<Interface> generate_graphone_ref_ignore_build(bool directed_graph){
    ...
    return unique_ptr<Interface>( new LiveGraphDriver(directed_graph, true));
}
std::unique_ptr<Interface> generate_livegraph_rw(bool directed_graph)
{ // read-write transactions for Graphalytics
    return unique_ptr<Interface>( new LiveGraphDriver(directed_graph, false));
}
#endif
...
#if defined(HAVE_HALB)
    std::unique_ptr<Interface> generate_OurIdea(bool directed_graph)
    {

        // cout << "Running HALB with block size: " << config.block_size() << endl;
        auto& config = configuration();
        // config.
        bool nodeleteooo = false;
        if((!config.is_out_of_order_workload()) && (!config.is_duplicate_support()) && (config.get_update_log().empty()))
        {
            nodeleteooo = true;
        }

        return std::unique_ptr<Interface>{ new HalbDriver(config.num_threads(THREADS_WRITE), config.is_out_of_order_workload(), config.is_duplicate_support(), config.is_mixed_workload(), nodeleteooo)};
    }
#endif
#if defined(HAVE_SORTLEDTONV2)
...
#endif
#if defined(HAVE_HALB)
        result.emplace_back("our", "HALB", &generate_OurIdea);
#endif
#if defined(HAVE_SORTLEDTONV2)
 ...
 *****************************************************************************/
    ...
    uint64_t Interface::on_thread_destroy(int thread_id){ } ;
        ...
        if(force){
            ...
            for(uint64_t i = 0; i < array_sz; i++){
                if(A[i].m_weight >= 0){ // insert
                    graph::WeightedEdge edge{A[i].m_source, A[i].m_destination, A[i].m_weight,A[i].s_timestamp};
                    ...
                    if ( ! remove_edge(edge,0) ){ // avoid infinite loops/waits
                        auto op = [this](graph::Edge edge){ return remove_edge(edge,0); };
                        batch_try_again(op, edge);
                    }
                }
            }
            ...
            for(uint64_t i = 0; i < array_sz; i++){
                if(A[i].m_weight >= 0){ // insert
                    result &= add_edge(graph::WeightedEdge{A[i].m_source, A[i].m_destination, A[i].m_weight,A[i].s_timestamp});
                } else { // remove
                    result &= remove_edge(graph::Edge{A[i].m_source, A[i].m_destination},0);
                }
            }
        }
