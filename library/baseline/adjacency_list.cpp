...
bool AdjacencyList::add_edge_v2(gfe::graph::WeightedEdge e, int thread_id) {
    COUT_DEBUG("edge: " << e);
    if(e.source() == e.destination()) INVALID_ARGUMENT("Cannot insert an edge with the same source and destination: " << e);
...
bool AdjacencyList::remove_edge(graph::Edge e, int thread_id){
    scoped_lock<mutex_t> lock(m_mutex);
    return delete_edge0(e);
}
bool AdjacencyList::remove_edge_v2(gfe::graph::WeightedEdge e, int thread_id)
{
